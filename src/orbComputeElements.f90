   ...
   SUBROUTINE orbComputeElements(t2,ecc,perh,xob,pre)
      ...
      DO j=1,neff
         ...
         xes = xes + ae(j)*SIN(arg)   ! jth term of ecc*SIN(Pi) with amp. M(j)
         xec = xec + ae(j)*COS(arg)   ! jth term of ecc*Cos(Pi) with amp. M(j)
      END DO
      ...
      ! defintion omegat = Pi + Psi, where Psi is the annual general 
      ! precession in longitude. 
      ! Pi is determined from ecc*SIN(Pi) and ecc*Cos(Pi):
      IF (ABS(xec) > 1.0E-08) THEN
         rp = ATAN(xes/xec)
         IF(xec < 0.0) THEN 
            rp = rp + c_pi
         ELSE IF (xes < 0.0) THEN 
            rp = rp + 2.0*c_pi
         END IF 
      ELSE
         IF (xes < 0.0) THEN 
